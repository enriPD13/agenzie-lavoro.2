// JobApp - CPI Module (stile Agenzie)
const CPI = {
  allCPI: [],
  favorites: [],
  favoriteSedi: [],
  ...
  render() {
    ...
    // Render province come "agenzie"
    grid.innerHTML = this.filteredProvinces.map(p => `
      <div class="bg-white rounded-3xl overflow-hidden shadow-sm">
        ...
        <div class="p-5">
          <div class="flex items-start justify-between mb-2">
            ...
            <button onclick="CPI.toggleFavorite('${p.sigla}')" class="ml-2 w-10 h-10 rounded-full flex items-center justify-center ${this.isFavorite(p.sigla) ? 'bg-yellow-100' : 'bg-gray-100'}">
              <svg class="w-6 h-6 ${this.isFavorite(p.sigla) ? 'text-yellow-500' : 'text-gray-400'}" fill="${this.isFavorite(p.sigla) ? 'currentColor' : 'none'}" stroke="currentColor" viewBox="0 0 24 24">
          ...
          <div class="flex gap-2 mb-3">
            ...
            <span class="text-xs text-gray-500">${p.sedi.length} sed${p.sedi.length === 1 ? 'e' : 'i'}</span>
          </div>
          
          <p class="text-sm text-gray-600 mb-4">Centri Per l'Impiego della provincia di ${p.nome}</p>
  ...
  openSedi(provinciaSigla) {
    ...
    document.getElementById('cpiSediCount').textContent = `${provincia.sedi.length} sed${provincia.sedi.length === 1 ? 'e' : 'i'} CPI`;
      ...
      html += `
        <div class="bg-gray-50 rounded-2xl p-4 mb-4">
          <div class="flex items-start justify-between mb-2">
            ...
            <button onclick="CPI.toggleSedeFavorite('${sedeId}', '${provincia.nome}', '${s.citta}')" class="w-8 h-8 rounded-full flex items-center justify-center ${isFav ? 'bg-yellow-100' : 'bg-gray-100'}">
          ...
          <div class="text-sm text-gray-600 space-y-2">
            ...
            ${s.orari ? `<div class="flex items-center gap-2 text-gray-600"><svg class="w-4 h-4 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>${s.orari}</div>` : ''}
    ...
    document.getElementById('cpiSediContent').innerHTML = html;
