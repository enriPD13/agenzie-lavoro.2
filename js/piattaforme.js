    ...
    return `
      <div class="bg-white rounded-3xl overflow-hidden shadow-sm">
        <div class="h-36 bg-gradient-to-br from-orange-50 to-red-50 flex items-center justify-center p-6">
          ${p.logo ? `<img src="${p.logo}" alt="${p.nome}" class="max-h-20 max-w-full object-contain"/>` : `<div class="text-6xl">📢</div>`}
        </div>
        
        <div class="p-5">
          <div class="flex items-start justify-between mb-2">
            <h3 class="text-xl font-bold flex-1">${p.nome}</h3>
            <button onclick="Piattaforme.toggleFavorite('${p.id}')" class="ml-2 w-10 h-10 flex items-center justify-center -mr-2">
              <svg class="w-5 h-5 ${this.isFavorite(p.id) ? 'text-orange-600' : 'text-gray-400'}" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          ${p.descrizione ? `<p class="text-sm text-gray-600 mb-4">${p.descrizione}</p>` : `<p class="text-sm text-gray-400 mb-4 italic">Descrizione non ancora disponibile</p>`}
